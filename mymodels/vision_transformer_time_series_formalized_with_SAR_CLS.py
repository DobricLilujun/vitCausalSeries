...
#     def __init__(self, in_channels, cls_class=2):
#         super(Classifier, self).__init__()
#         self.conv1 = nn.Conv2d(
#             in_channels=in_channels, out_channels=64, kernel_size=3, padding=1
#         )
#         self.conv2 = nn.Conv2d(
#             in_channels=64, out_channels=cls_class + 1, kernel_size=3, padding=1
#         )
...
class Classifier(nn.Module):
    def __init__(self, in_channels, cls_class=2):
        ...
        self.conv5 = nn.Conv2d(512, cls_class + 1, kernel_size=3, padding=1)
...
class VisionTransformerTimeSeriesFormalizedWithSARCLS(nn.Module):
    def __init__(
        self,
        avrg_img_size=320,
        patch_size=10,
...
#     def __init__(self, in_chans=11, embed_dim=64, patch_size=5, Kt=2, St=2):
#         super().__init__()
#         self.proj = nn.Conv3d(
...
#                      in_chans=11, embed_dim=embed_dim, patch_size=self.patch_size[0], Kt=2, St=2
#                 )
#         else:
#             self.patch_embed = ShiftedPatchTokenization(
#                 in_chans, embed_dim, merging_size=self.patch_size, is_pe=True
...
#         if self.use_time_embed:
#             self.head = nn.Linear(
#                 self.num_features*3, in_chans * self.patch_size[0] * self.patch_size[1]
#             )
