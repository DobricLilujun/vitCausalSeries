...
class PatchDecodeTimeSeries(nn.Module):
    def __init__(self, patch_size, embed_dim, out_chans, time_span=1):
        super().__init__()
...
class PatchEmbedTimeSeries(nn.Module):
    ...
    def __init__(self, patch_size, in_chans, embed_dim, time_span=1):
        super().__init__()
...
class VisionTransformerTimeSeriesFormalized(nn.Module):
    def __init__(
        ...
        #     patch_size, in_chans, embed_dim, time_span=1 #6
        # )
        if not is_SPT:
            if not is_CSA:
                ...
                self.patch_embed = PatchEmbedTimeSeries(
                    ...
                    in_chans=in_chans,
                    embed_dim=embed_dim,
                    time_span=in_chans // in_chans,  # 11 -> in_chans
                )
        ...
        if self.use_time_embed:
            self.time_embed = nn.Parameter(
                torch.zeros(
                    ...
                    self.in_chans // in_chans,  # Temporal height (H_t) 11 -> in_chans
                )
            )
    ...
    def seq2img(self, x, img_size, input_channel=1):
        ...
        if not self.is_CSA:
            ...
            time_span = self.out_chans // input_channel
            x = x.view(
